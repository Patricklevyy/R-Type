...
namespace poc_game
{
    ...
    {
        _eventBus.subscribe(POC_GAME_ACTIONS::START_LISTEN_EVENT,
            [this](const std::vector<std::any> &args) {
                (void) args;
                _event_window_system.startListening(_ecs._components_arrays);
            });
        _eventBus.subscribe(POC_GAME_ACTIONS::STOP_LISTEN_EVENT,
            [this](const std::vector<std::any> &args) {
                (void) args;
                _event_window_system.stopListening();
            });
        _eventBus.subscribe(POC_GAME_ACTIONS::RENDER_WINDOW,
            [this](const std::vector<std::any> &args) {
                (void) args;
                _render_window_system.render(_ecs._components_arrays);
            });
        _eventBus.subscribe(POC_GAME_ACTIONS::CREATE_CLIENT,
            [this](const std::vector<std::any> &args) {
        ...
        _eventBus.subscribe(POC_GAME_ACTIONS::CREATE_PLAYER,
            [this](const std::vector<std::any> &args) {
                ecs::udp::Message message =
                    std::any_cast<std::reference_wrapper<ecs::udp::Message>>(
                        args[0])
        ...
        _eventBus.subscribe(
            POC_GAME_ACTIONS::START, [this](const std::vector<std::any> &args) {
                ecs::udp::Message message =
                    std::any_cast<std::reference_wrapper<ecs::udp::Message>>(
                        args[0])
                        .get();
        ...
        _eventBus.subscribe(
            POC_GAME_ACTIONS::JUMP, [this](const std::vector<std::any> &args) {
                ecs::udp::Message message =
                    std::any_cast<std::reference_wrapper<ecs::udp::Message>>(
                        args[0])
                        .get();
        ...
        _eventBus.subscribe(POC_GAME_ACTIONS::UPDATE_POSITIONS_FROM_SERVER,
            [this](const std::vector<std::any> &args) {
                try {
                    ...
                    std::list<std::tuple<std::size_t, float, float>> entities =
                        Utils::parse_update(message.params);

                    while (!entities.empty()) {
                        _update_entity_system.updateEntity(
                            _ecs._components_arrays,
                            ecs_server_to_client[std::get<0>(entities.front())],
                            std::get<1>(entities.front()),
                            std::get<2>(entities.front()));
                        entities.pop_front();
                    }
        ...
        _eventBus.subscribe(POC_GAME_ACTIONS::KILL_ENTITY,
            [this](const std::vector<std::any> &args) {
                try {
        ...
        _eventBus.subscribe(POC_GAME_ACTIONS::FAIL_LEVEL,
            [this](const std::vector<std::any> &args) {
                try {
                    auto &message = std::any_cast<
                        std::reference_wrapper<ecs::udp::Message>>(args[0])
        ...
        _eventBus.subscribe(POC_GAME_ACTIONS::SPAWN_PIPE,
            [this](const std::vector<std::any> &args) {
                try {
                    auto &message = std::any_cast<
                        std::reference_wrapper<ecs::udp::Message>>(args[0])
        ...
        // _eventBus.subscribe(RTYPE_ACTIONS::WIN_LEVEL, [this](const
        // std::vector<std::any> &args) {
        //     try {
        //         auto &message =
        //         std::any_cast<std::reference_wrapper<ecs::udp::Message>>(args[0]).get();
        ...
        //     }
        // });
        // _eventBus.subscribe(RTYPE_ACTIONS::REMOVE_ATH, [this](const
        // std::vector<std::any> &args) {
        //     (void)args;

        //     _ath_system.removeLevels(_ecs);
        // });
        // _eventBus.subscribe(RTYPE_ACTIONS::CREATE_PLAYER, [this](const
        // std::vector<std::any> &args) {
        //     ecs::udp::Message message =
        //     std::any_cast<std::reference_wrapper<ecs::udp::Message>>(args[0]).get();
