...
namespace poc_game
{
    size_t Room::create_player(std::pair<float, float> positions)
    {
        ...
        _ecs.addComponents<Hitbox>(index, Hitbox(HitboxFactory::createHitbox(SPRITES::PLAYER)));
    ...
    void Room::spawn_pipes()
    {
        ...
        float elapsed_time = std::chrono::duration<float>(current_time - _last_spawn_time).count();
        float SPACE = 320;

        if (elapsed_time >= PIPE_SPAWN_INTERVAL) {
            size_t index = getNextIndex();
            std::pair<int, int> positions = PipeFactory::getMonsterSpawnCoordinates(_window_width, _window_height, _random_number);

            _ecs.addComponents<ecs::Position>(index, ecs::Position(positions.first, positions.second - SPACE));
            ...
            _ecs.addComponents<Hitbox>(index, Hitbox(HitboxFactory::createHitbox(SPRITES::PIPE_UP)));
            _ecs.addComponents<Ennemies>(index, Ennemies());

            send_client_new_pipe(index, positions.first, positions.second - SPACE, SPRITES::PIPE_DOWN);
            ...
            send_client_new_pipe(index, positions.first, positions.second + SPACE, SPRITES::PIPE_UP);
