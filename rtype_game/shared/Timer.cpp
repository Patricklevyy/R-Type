...
namespace rtype
{
    ...
    void Timer::init(const std::string &configFile, bool isServer)
    {
        ...
        try {
            ...
            if (isServer) {
                if (cfg.exists("server.tps")) {
                    ...
                    if (tps > 0) {
                        ...
                        std::cout << "Serveur: TPS configuré à " << tps << " ("
                                  << (1.0f / tps) << " secondes par tick)"
                                  << std::endl;
                    } else {
                        throw rtype::ERROR::InvalidTpsOrFpsExceptions(
                            "Server TPS doit être supérieur à 0");
                    }
                } else {
                    std::cerr << "Le paramètre 'server.tps' est manquant dans "
                                 "la configuration."
                              << std::endl;
                }
            } else {
                if (cfg.exists("client.tps")) {
                    ...
                    if (tps > 0) {
                        _tps = tps;
                        tps_duration = std::chrono::duration<float>(1.0f / tps);
                        std::cout << "Client: TPS configuré à " << tps << " ("
                                  << (1.0f / tps) << " secondes par tick)"
                                  << std::endl;
                    } else {
                        throw rtype::ERROR::InvalidTpsOrFpsExceptions(
                            "Client tps doit être supérieur à 0");
                    }
                } else {
                    std::cerr << "Le paramètre 'client.tps' est manquant dans "
                                 "la configuration."
                              << std::endl;
                }
                if (cfg.exists("client.fps")) {
                    ...
                    if (fps > 0) {
                        ...
                        std::cout << "Client: FPS configuré à " << fps << " ("
                                  << (1.0f / fps) << " secondes par image)"
                                  << std::endl;
                    } else {
                        throw rtype::ERROR::InvalidTpsOrFpsExceptions(
                            "Client fps doit être supérieur à 0");
                    }
                } else {
                    std::cerr << "Le paramètre 'client.fps' est manquant dans "
                                 "la configuration."
                              << std::endl;
                }
            }
            ...
            std::cerr << "Erreur de syntaxe dans le fichier de configuration ("
                      << e.getFile() << ":" << e.getLine()
                      << ") : " << e.getError() << std::endl;
