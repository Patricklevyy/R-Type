...
namespace rtype
{
    ...
    {
        if (type == SPRITES::PLAYER_SIMPLE_MISSILE || type == SPRITES::PLAYER_CHARGED_SHOOT)
            return true;
        return false;
    }
    ...
    std::string Utils::bonusInfoToString(const std::pair<float, float> &position, int direction_x, int direction_y, SPRITES sprite, int velocity)
    {
        std::string info = "";

        info += "x=" + std::to_string(position.first) + ";y=" + std::to_string(position.second);
        info += ";dir_x=" + std::to_string(direction_x) + ";dir_y=" + std::to_string(direction_y);
        info += ";v=" + std::to_string(velocity);
    ...
    std::list<std::pair<std::size_t, std::pair<float, float>>> Utils::parse_update(const std::string &params) {
    ...
    std::vector<std::tuple<std::pair<float, float>, int, int>> Utils::parseUpdateEntities(const std::string &message)
    {
    ...
    std::tuple<std::pair<float, float>, int, int> Utils::parsePositionAndRoomPort(const std::string &input)
    {
