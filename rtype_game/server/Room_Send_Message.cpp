...
namespace rtype
{
    ...
    void Room::send_client_new_projectile(
        size_t index_ecs_server, float x, float y, SPRITES sprite_type)
    {
        std::vector<char> response;
        ecs::udp::Message responseMessage;
        ...
        std::cout << "JE SHOTT"
                ...
                + ";type=" + std::to_string(sprite_type)
                  << std::endl;
        responseMessage.params = "x=" + std::to_string(x) + ";y="
            + std::to_string(y) + ";type=" + std::to_string(sprite_type);
    ...
    void Room::sendUpdate()
    {
        ...
        auto &healths = std::any_cast<ecs::SparseArray<Health> &>(
            _ecs._components_arrays[typeid(Health)]);
    ...
    std::string Room::sendExistingEntities()
    {
        auto &positions = std::any_cast<ecs::SparseArray<ecs::Position> &>(
            _ecs._components_arrays[typeid(ecs::Position)]);
        auto &sprite = std::any_cast<ecs::SparseArray<SpriteId> &>(
            _ecs._components_arrays[typeid(SpriteId)]);
        ...
        for (size_t i = 0; i < positions.size(); ++i) {
            if (positions[i].has_value() && sprite[i].has_value()) {
                ...
                updateMessage += std::to_string(positions[i].value()._pos_x)
                    ...
                    + std::to_string(i) + "," + std::to_string(spriteId) + ";";
            }
        }
    ...
    void Room::send_client_new_monster(
        size_t index_ecs_server, float x, float y, int type)
    {
        std::vector<char> response;
        ecs::udp::Message responseMessage;
