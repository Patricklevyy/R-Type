...
namespace rtype
{
    ...
    class MonsterMovementSystem {
      public:
        /**
         * @brief Updates monster positions based on their velocity and
         * direction.
         ...
         */
        void moveMonsters(ecs::ECS &ecs, int window_width, int window_height,
            RandomNumber randomizer)
        {
            auto &positions = std::any_cast<ecs::SparseArray<ecs::Position> &>(
                ecs._components_arrays[typeid(ecs::Position)]);
            auto &directions =
                std::any_cast<ecs::SparseArray<ecs::Direction> &>(
                    ecs._components_arrays[typeid(ecs::Direction)]);
            auto &velocities = std::any_cast<ecs::SparseArray<ecs::Velocity> &>(
                ecs._components_arrays[typeid(ecs::Velocity)]);
            auto &monsters = std::any_cast<ecs::SparseArray<Monster> &>(
                ecs._components_arrays[typeid(Monster)]);

            for (size_t i = 0; i < monsters.size(); ++i) {
                if (positions[i].has_value() && velocities[i].has_value()
                    && directions[i].has_value() && monsters[i].has_value()) {
                    switch (monsters[i].value()._monster) {
                        ...
                        case SPRITES::ADVANCED_MONSTER:
                            if (randomizer.generateRandomNumbers(0, 100) < 5) {
                                int randX =
                                    randomizer.generateRandomNumbers(0, 2);
                                if (randX == 0)
                                    directions[i].value()._x =
                                        ecs::direction::LEFT;
                                else if (randX == 1)
                            ...
                            if (positions[i].value()._pos_x <= 200) {
                                directions[i].value()._x =
                                    ecs::direction::RIGHT;
                            } else if (positions[i].value()._pos_x
                                >= window_width - 200) {
