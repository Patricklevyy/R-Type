    ...
    namespace rtype
    {
        class AsteroideSystem {
            public:
                ...
                std::list<std::tuple<size_t, std::pair<std::pair<float, float>, std::pair<float, float>>, SPRITES>> spwan_asteroide(RandomNumber randomizer, std::shared_ptr<GameplayFactory> gameplayFactory, int window_height, int window_width) {
                    std::list<std::tuple<size_t, std::pair<std::pair<float, float>, std::pair<float, float>>, SPRITES>> asteroids;

                    if (gameplayFactory->getDifficulty() == DIFFICULTY::EASY)
                        return asteroids;
