...
{
    EXPECT_THROW(
        test_no_config_file(), ecs::ERROR::CantReadConfigFileExceptions);
}

TEST(ConnexionTest, NoSecretKey)
{
    EXPECT_THROW(test_no_secret_key(),
        ecs::ERROR::MissingSecretKeyInConfigFileException);
}

TEST(ConnexionTest, WrongBufferSize)
{
    EXPECT_THROW(
        test_wrong_buffer_size(), ecs::ERROR::WrongBufferSizeExceptions);
}
...
{
    EXPECT_THROW(test_send_message_with_no_init(),
        ecs::ERROR::InvalidAdressFormatExceptions);
}

TEST(SendMessageTest, SendMessageToBig)
{
    EXPECT_THROW(
        test_send_message_to_big(), ecs::ERROR::MessageTooBigExceptions);
}
...
{
    EXPECT_THROW(add_registry_already_exist(),
        ecs::ERROR::ComponentTypeAlreadyExistException);
}

TEST(EcsTests, ComponentDontExist)
{
    EXPECT_THROW(
        add_component_dont_exist(), ecs::ERROR::ComponentNotFoundExceptions);
}
