        ...
        while (!_events.empty()) {
            ...
            switch (event.type) {
                ...
                case sf::Event::KeyPressed:
                    ...
                    if (event.key.code == sf::Keyboard::D && !_in_menu) {
                        std::tuple<ecs::direction, ecs::direction, size_t> _x_y(
                            ...
                            _ecs.getIndexPlayer());
                        send_server_player_direction(
                            ecs::direction::RIGHT, ecs::direction::NO_CHANGE);
                        _eventBus.emit(RTYPE_ACTIONS::UPDATE_PLAYER_DIRECTION,
                            std::ref(_x_y));
                    }
                    ...
                    if (event.key.code == sf::Keyboard::A) {
                        ...
                        mess.params = "room_name=room1;client_name=jean;x="
                    ...
                    if (event.key.code == sf::Keyboard::M) {
                        ...
                        if (_udpClient->sendMessageToDefault(buffer)) {
                            std::cout << "Request sent to create a monster"
                                      << std::endl;
                        } else {
                    ...
                    if (event.key.code == sf::Keyboard::Num1) {
                        _set_filter_system.setFilter(
                            _ecs._components_arrays, FILTER_MODE::Neutral);
                    }
                    if (event.key.code == sf::Keyboard::Num2) {
                        _set_filter_system.setFilter(
                            _ecs._components_arrays, FILTER_MODE::Inverted);
                    }
                    if (event.key.code == sf::Keyboard::Num3) {
                        _set_filter_system.setFilter(
                            _ecs._components_arrays, FILTER_MODE::Protanopia);
                    }
                    if (event.key.code == 51) {
                        _set_filter_system.setFilter(
                            _ecs._components_arrays, FILTER_MODE::Deuteranopia);
                    }
                    if (event.key.code == sf::Keyboard::Num5) {
                        _set_filter_system.setFilter(
                            _ecs._components_arrays, FILTER_MODE::Tritanopia);
                    }
                    break;
                case sf::Event::KeyReleased:
                    if ((event.key.code == sf::Keyboard::D
                        ...
                        std::tuple<ecs::direction, ecs::direction, size_t> _x_y(
                            ...
                            ecs::direction::NO_CHANGE, _ecs.getIndexPlayer());
                        send_server_player_direction(
                            ecs::direction::NO_DIRECTION,
                            ecs::direction::NO_CHANGE);
                        _eventBus.emit(RTYPE_ACTIONS::UPDATE_PLAYER_DIRECTION,
                            std::ref(_x_y));
                    }
                    if ((event.key.code == sf::Keyboard::Z
                        ...
                        std::tuple<ecs::direction, ecs::direction, size_t> _x_y(
                            ...
                            ecs::direction::NO_DIRECTION,
                            _ecs.getIndexPlayer());
                        send_server_player_direction(ecs::direction::NO_CHANGE,
                            ecs::direction::NO_DIRECTION);
                        _eventBus.emit(RTYPE_ACTIONS::UPDATE_PLAYER_DIRECTION,
                            std::ref(_x_y));
                    }
                ...
                case sf::Event::MouseButtonPressed: {
                    std::pair<bool, LEVELS> isLevelChosen =
                    ...
                    } else if (_ath_system.isLooseOrWinClicked(
