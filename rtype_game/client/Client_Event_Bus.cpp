...
namespace rtype
{
    ...
    {
        _eventBus.subscribe(RTYPE_ACTIONS::GET_WINDOW_EVENT,
            [this](const std::vector<std::any> &args) {
                (void) args;
                _events = _event_window_system.fetchEvents();
            });
        _eventBus.subscribe(RTYPE_ACTIONS::START_LISTEN_EVENT,
            [this](const std::vector<std::any> &args) {
                (void) args;
                _event_window_system.startListening(_ecs._components_arrays);
            });
        _eventBus.subscribe(RTYPE_ACTIONS::STOP_LISTEN_EVENT,
            [this](const std::vector<std::any> &args) {
                (void) args;
                _event_window_system.stopListening();
            });
        _eventBus.subscribe(RTYPE_ACTIONS::CREATE_CLIENT,
            [this](const std::vector<std::any> &args) {
        ...
        _eventBus.subscribe(rtype::RTYPE_ACTIONS::UPDATE_PLAYER_POSITION,
            [this](const std::vector<std::any> &args) {
                (void) args;
                _position_system.updatePlayerPositions(_ecs._components_arrays,
                    _timer->getTps(), _ecs.getIndexPlayer(), _window_width,
                    _window_height);
            });
        _eventBus.subscribe(rtype::RTYPE_ACTIONS::UPDATE_POSITION,
            [this](const std::vector<std::any> &args) {
                try {
                    ...
                    std::list<
                        std::tuple<std::size_t, std::pair<float, float>, int>>
                        entities =
                            Command_checker::parse_update(message.params);
        ...
        _eventBus.subscribe(rtype::RTYPE_ACTIONS::CREATE_TEAMMATE,
            [this](const std::vector<std::any> &args) {
                try {
                    auto &message = std::any_cast<
                        std::reference_wrapper<ecs::udp::Message>>(args[0])
                    ...
                    std::string x_part =
                        message.params.substr(0, separator_pos);
                    std::string y_part =
                        message.params.substr(separator_pos + 1);
        ...
        _eventBus.subscribe(rtype::RTYPE_ACTIONS::RENDER_WINDOW,
            [this](const std::vector<std::any> &args) {
                (void) args;
        ...
        _eventBus.subscribe(RTYPE_ACTIONS::KILL_ENTITY,
            [this](const std::vector<std::any> &args) {
                try {
                    ecs::udp::Message message = std::any_cast<
                        std::reference_wrapper<ecs::udp::Message>>(args[0])
